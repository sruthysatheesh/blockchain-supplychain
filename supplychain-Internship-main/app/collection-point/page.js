...
import { Leaf, LogOut, Package, Truck, Loader2, Search, X, QrCode, CheckCircle as CheckCircleIcon } from "lucide-react";
...
export default function CollectionPointDashboard() {
  ...
  // Static Data
  const shippableRoles = ['Warehouse', 'Processing Unit', 'Retailer'];
  ...
  const productsInStock = useMemo(() => {
    ...
    return allProducts.filter(p => 
        ...
        p.currentState.toString() === '2' // State.AT_COLLECTION_POINT
    );
  }, [allProducts, user]);
  ...
  const productsShipped = useMemo(() => {
    ...
    const shipped = allProducts.filter(p =>
        p.history.length > 0 &&
        ethers.getAddress(p.history[0].actor) === userAddress && // Originated from this actor
        p.currentState.toString() !== '2' // Not still AT_COLLECTION_POINT
    );
  ...
  if (!user || user.role !== "Collection Point") return <div className="flex flex-col justify-center items-center min-h-screen text-center"><p className="text-xl mb-4">Access Denied</p><Link href="/" className="bg-blue-600 text-white hover:bg-blue-700 rounded-full px-8 py-3 font-semibold">Go Back Home</Link></div>;
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <header className="w-full py-4 px-6 bg-white shadow-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
                ...
                <span className="font-bold text-lg">AgriChain Collection Point</span>
            </Link>
      ...
      <main className="flex-grow p-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold text-gray-800 mb-6">Collection Point Dashboard</h1>
              ...
              {activeTab === 'incoming' && (
                <div className="space-y-4">
                  {incomingProducts.map(p => {
                    const sender = actorMap.get(ethers.getAddress(p.currentOwner));
                    const shippedEvent = p.history[p.history.length - 1];
                    return (
                        <div key={p.productId.toString()} className="bg-gray-50 border rounded-lg p-4 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <Truck className="w-8 h-8 text-yellow-500 flex-shrink-0"/>
                                <div className="flex-grow">
                                    <div><span className="font-bold">{p.name}</span> <span className="text-xs text-gray-400">#{p.productId.toString()}</span></div>
                                    <p className="text-sm text-gray-600">{p.quantity.toString()} {p.unit}</p>
                                    <p className="text-xs text-gray-500 mt-1">Shipped From: <span className="font-medium text-gray-700">{sender?.name || 'Unknown'}</span></p>
                                </div>
                            </div>
                            <button onClick={() => handleReceiveProduct(p.productId)} disabled={isProcessing} className="inline-flex items-center gap-2 bg-green-600 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-green-700 disabled:bg-green-300">
                                {isProcessing ? <Loader2 className="w-4 h-4 animate-spin"/> : <CheckCircleIcon className="w-4 h-4"/>} Receive
                            </button>
                        </div>
                    );
                  })}
              ...
              {activeTab === 'stock' && (
                <div className="space-y-4">
                  {productsInStock.map(p => {
                    const receivedEvent = p.history.find(e => e.details.toLowerCase().includes('received'));
                    const senderEvent = receivedEvent ? p.history[p.history.indexOf(receivedEvent) - 1] : null;
                    const sender = senderEvent ? actorMap.get(ethers.getAddress(senderEvent.actor)) : null;
                    return (
                        <div key={p.productId.toString()} className="bg-gray-50 border rounded-lg p-4 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-grow">
                                    <div><span className="font-bold">{p.name}</span> <span className="text-xs text-gray-400">#{p.productId.toString()}</span></div>
                                    <p className="text-sm text-gray-600">{p.quantity.toString()} {p.unit}</p>
                                    <p className="text-xs text-gray-500 mt-1">Received From: <span className="font-medium text-gray-700">{sender?.name || 'Unknown'}</span></p>
                                </div>
                            </div>
